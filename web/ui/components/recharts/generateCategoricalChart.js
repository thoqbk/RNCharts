...
var generateCategoricalChart = function generateCategoricalChart(_ref) {
  ...
  var CategoricalChartWrapper = (_temp = _class = function (_Component) {
    ...
    */
    ...
    //--------------------------------------------------------------------------
    // Support WebViewBridge. Tho Q Luong Oct 1, 2017
    var self = _this7;

    this.handleTouchMove = function (e) {
      self.cancelEnablingTooltipTimer();
      if(self.state.isTooltipActive !== true) {
        return;
      }
      ...
      window.touchPoints.push(point);
      var CHECKED_POINTS = 10;
      var TOLERANCE = 5;
      if(window.touchPoints.length < CHECKED_POINTS) {
        return;
      }
      var first = window.touchPoints[window.touchPoints.length - CHECKED_POINTS];
      var last = window.touchPoints[window.touchPoints.length - 1];
    ...
    this.handleTouchStart = function(e) {
      ...
      var b = e.changedTouches != null && e.changedTouches.length > 0;
      var b1 = self.state.isTooltipActive !== true;
      if(b && b1) {
        self.cancelEnablingTooltipTimer();
        var clickEvent = {
        ...
        // alert(clickEvent);
        window.enablingTooltipTimer = setTimeout(function() {
          self.lock();
          self.handleMouseMove(clickEvent);
          window.enablingTooltipTimer = null;
        }, 1000);
      }
    }
    ...
    this.handleTouchEnd = function() {
      ...
      if(touchEndTime - window.touchStartTime < 200 && self.state.isTooltipActive) {
        setTimeout(function() {
          self.setState({
            isTooltipActive: false
          });
    ...
    this.cancelEnablingTooltipTimer = function() {
      if(window.enablingTooltipTimer != null) {
        clearTimeout(window.enablingTooltipTimer);
        window.enablingTooltipTimer = null;
      }
    }
    //--------------------------------------------------------------------------
